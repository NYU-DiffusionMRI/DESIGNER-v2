...
ground_truth = json.load(open("tests/ground_truth_statistics/D2_wo_bids.json"))


@pytest.fixture(scope="module")
def paths():
    data_dir = Path("tests/data/D2/")
    tmp_dir = Path("tests/tmp_D2_wo_bids/")
    ...
    return {
        ...
        "phase_encoding_image": data_dir / "pa_ds2.nii.gz",
...
def run_pipeline(paths):
    ...
    try:
        ...
        ret = subprocess.run(["designer", "-set_seed", "-eddy", "-rpe_pair", str(paths["phase_encoding_image"].resolve()), "-pf", "6/8", "-pe_dir", "AP", "-mask", "-scratch", str(processing_dir), "-nocleanup", dwi_args, str(designer_image_path)])
