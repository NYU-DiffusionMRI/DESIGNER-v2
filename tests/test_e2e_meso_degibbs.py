...
from tests.e2e_runner_factory import prepare_meso_degibbs_e2e_runner
from tests.types import DWIStage, DiffusionModelType, StatsDict
from tests.utils import assert_stats


ground_truth = json.load(open("tests/benchmark/D3_meso_degibbs.json"))
# ground_truth = json.load(open("benchmark_windows_x86_64/D3_meso_degibbs.json"))

test_dir = Path("tests/")
data_dir = test_dir / "data/D3/"
scratch_dir = test_dir / "tmp_D3/"


@pytest.fixture(scope="module", autouse=True)
def runner(request: pytest.FixtureRequest):
    test_runner: E2ERunner = prepare_meso_degibbs_e2e_runner(scratch_dir, data_dir)
