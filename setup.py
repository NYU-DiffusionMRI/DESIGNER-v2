...
class CustomBuildExt(build_ext):
    def run(self):
        ...
        self.extract_fftw_tar()  # Extract the tar file
        self.build_fftw()
        super().run()

    def clean_fftw(self):
        fftw_extract_dir = os.path.abspath("rpg_cpp/fftw-3.3.10")

        if os.path.exists(fftw_extract_dir):
            subprocess.check_call(["make", "clean"], cwd=fftw_extract_dir)

    def extract_fftw_tar(self):
        tar_path = os.path.abspath("dependencies/fftw-3.3.10.tar.gz")
        extract_path = os.path.abspath("rpg_cpp")

        if not os.path.exists(os.path.join(extract_path, "fftw-3.3.10")):
